...
# # not needed...
# # Used in _extract_layer_info --- used in lieu of tf.identity, perhaps better way for this? 
# def identity(input):
# 	return torch.matrix_power(input, n=0)
...
# IN PROGRESS: need to check how this plays into everything else
def _extract_layer_info(network_info, input_, nonlinearity):
    ...
    input_channels = int(input_.size()[-1])
    if nonlinearity is None:
        nonlinearity = torch.nn.Identity # torch.nn.Identity(*args, **kwargs)
...
# TODO: do we need this? research on how tensorflow uses these collections and how cache is used in this codebase....
def _create_cache(batch, cache_height, cache_width, channels):
    '''Creates a cache, which is used to avoid redundant computation.'''
    cache = torch.tensor(
        data=np.zeros((batch, cache_height, cache_width, channels)),
        dtype=torch.float32)
    # Reset the cache between generations.
    reset_cache = cache.assign(torch.zeros_like(cache))
    tf.add_to_collection(RESET_CACHE_COLLECTION, reset_cache) # if func is needed, need to change this
    return cache
...
# TODO: same thing as above, do we need this???
def reset_cache_op():
