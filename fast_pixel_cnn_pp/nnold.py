...
def concat_elu(x):
    ...
    axis = len(x.size()) - 3 # PyTorch ordering, https://github.com/pclucas14/pixel-cnn-pp/issues/11
...
# IN PROGRESS -- finish line 130 (log_probs = ...)
def discretized_mix_logistic_loss(x,l,sum_all=True):
    ...
    l = torch.view(l[:,:,:,nr_mix:], xs + [nr_mix*3])
    means = l[:,:,:,:,:nr_mix]
    log_scales = torch.max(l[:,:,:,:,nr_mix:2*nr_mix], -7.)[0] # why is tensor #2 equal to -7. ?
    coeffs = torch.nn.Tanh(l[:,:,:,:,2*nr_mix:3*nr_mix])
    x = torch.view(x, xs + [1]) + tf.zeros(xs + [nr_mix]) # here and below: getting the means and adjusting them based on preceding sub-pixels
    m2 = torch.view(means[:,:,:,1,:] + coeffs[:, :, :, 0, :] * x[:, :, :, 0, :], [xs[0],xs[1],xs[2],1,nr_mix])
    m3 = torch.view(means[:, :, :, 2, :] + coeffs[:, :, :, 1, :] * x[:, :, :, 0, :] + coeffs[:, :, :, 2, :] * x[:, :, :, 1, :], [xs[0],xs[1],xs[2],1,nr_mix])
    means = torch.cat(3,[torch.view(means[:,:,:,0,:], [xs[0],xs[1],xs[2],1,nr_mix]), m2, m3])
    centered_x = x - means
    inv_stdv = torch.exp(-log_scales)
    plus_in = inv_stdv * (centered_x + 1./255.)
    cdf_plus = torch.nn.Sigmoid(plus_in)
    min_in = inv_stdv * (centered_x - 1./255.)
    cdf_min = torch.nn.Sigmoid(min_in)
    ...
    log_pdf_mid = mid_in - log_scales - 2.* softplus(mid_in) # log probability in the center of the bin, to be used in extreme cases (not actually used in our code)
    ...
    log_probs = torch.max(log_probs,3)[0] + log_prob_from_logits(logit_probs)
    if sum_all:
...
# IN PROGRESS --- finish line 166 (sel = ...)
def sample_from_discretized_mix_logistic(l,nr_mix,seed=None):
    ...
    # note torch.rand will sample from [0, 1), which is what I believe was desired by the authors here; they wanted to exclude 0, so I am adding 1e-5 to the result (max is 0.9999 according to https://github.com/pytorch/pytorch/issues/17289)
    # might want to consider using self.uniform_(from=a,to=b) to establish firm bounds -- check this later:
    rand_unif = torch.empty(logit_probs.size()).uniform_(1e-5, 1. - 1e-5)
    sel = tf.one_hot(torch.argmax(logit_probs - torch.log(-torch.log(rand_unif)), 3), depth=nr_mix, dtype=tf.float32)  # index 1 of torch.max returns argmax indices

    sel = torch.view(sel, xs[:-1] + [1,nr_mix])
    # select logistic parameters
    means = torch.sum(l[:,:,:,:,:nr_mix]*sel,4)
    log_scales = torch.max(torch.sum(l[:,:,:,:,nr_mix:2*nr_mix]*sel,4), -7.)[0]
    coeffs = torch.sum(torch.nn.Tanh(l[:,:,:,:,2*nr_mix:3*nr_mix])*sel,4)
    # sample from logistic & clip to interval
    # we don't actually round to the nearest 8bit value when sampling
    
    if seed is not None:
        torch.manualSeed(seed + 1)
    u = torch.empty(means.size()).uniform_(1e-5, 1. - 1e-5)

    x = means + torch.exp(log_scales) * (torch.log(u) - torch.log(1. - u))
    x0 = torch.min(torch.max(x[:,:,:,0], -1.)[0], 1.)[0]
    x1 = torch.min(torch.max(x[:,:,:,1] + coeffs[:,:,:,0]*x0, -1.)[0], 1.)[0]
    x2 = torch.min(torch.max(x[:,:,:,2] + coeffs[:,:,:,1]*x0 + coeffs[:,:,:,2]*x1, -1.)[0], 1.)[0]
    return torch.cat([torch.view(x0,xs[:-1]+[1]), torch.view(x1,xs[:-1]+[1]), torch.view(x2,xs[:-1]+[1])], 3)
...
def down_shift(x):
    xs = int_shape(x)
    return torch.cat(1,[torch.zeros([xs[0],1,xs[2],xs[3]]), x[:,:xs[1]-1,:,:]])
    # return tf.concat(1,[tf.zeros([xs[0],1,xs[2],xs[3]]), x[:,:xs[1]-1,:,:]]) # tf

# COMPLETED
def right_shift(x):
    xs = int_shape(x)
    return torch.cat(2,[torch.zeros([xs[0],xs[1],1,xs[3]]), x[:,:,:xs[2]-1,:]])
    # return tf.concat(2,[tf.zeros([xs[0],xs[1],1,xs[3]]), x[:,:,:xs[2]-1,:]]) # tf

# IN PROGRESS -- check after modifying conv2d
@add_arg_scope
def down_shifted_conv2d(x, num_filters, filter_size=[2,3], stride=[1,1], **kwargs):
    x = F.pad(x, [[0,0],[filter_size[0]-1,0], [int((filter_size[1]-1)/2),int((filter_size[1]-1)/2)],[0,0]])
    return conv2d(x, num_filters, filter_size=filter_size, pad='VALID', stride=stride, **kwargs) # check this after modifying conv2d()
    # # tf:
    # x = tf.pad(x, [[0,0],[filter_size[0]-1,0], [int((filter_size[1]-1)/2),int((filter_size[1]-1)/2)],[0,0]])
    # return conv2d(x, num_filters, filter_size=filter_size, pad='VALID', stride=stride, **kwargs)

# IN PROGRESS -- check after modifying deconv2d; no changes made here (nothing tf-specific, everything is np)
@add_arg_scope
...
def down_right_shifted_conv2d(x, num_filters, filter_size=[2,2], stride=[1,1], **kwargs):
    x = F.pad(x, [[0,0],[filter_size[0]-1, 0], [filter_size[1]-1, 0],[0,0]])
    return conv2d(x, num_filters, filter_size=filter_size, pad='VALID', stride=stride, **kwargs)
    # # tf
