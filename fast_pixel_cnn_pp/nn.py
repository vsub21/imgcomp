...
def discretized_mix_logistic_loss(x,l,sum_all=True):
    ...
    means = l[:,:,:,:,:nr_mix]
    log_scales = tf.max(l[:,:,:,:,nr_mix:2*nr_mix], -7.) # why is tensor #2 equal to -7. ?
    ...
    means = torch.cat(3,[torc.view(means[:,:,:,0,:], [xs[0],xs[1],xs[2],1,nr_mix]), m2, m3])
    centered_x = x - means
    ...
    log_probs = torch.max(log_probs,3) + log_prob_from_logits(logit_probs)
    if sum_all:
...
def sample_from_discretized_mix_logistic(l,nr_mix,seed=None):
    ...
    sel = tf.one_hot(torch.max(logit_probs - torch.log(-torch.log(rand_unif)), 3)[1], depth=nr_mix, dtype=tf.float32)  # index 1 of torch.max returns argmax indices

    sel = torch.view(sel, xs[:-1] + [1,nr_mix])
    # select logistic parameters
    means = torch.sum(l[:,:,:,:,:nr_mix]*sel,4)
    log_scales = torch.maximum(tf.torch(l[:,:,:,:,nr_mix:2*nr_mix]*sel,4), -7.)
    coeffs = tf.sum(torch.nn.Tanh(l[:,:,:,:,2*nr_mix:3*nr_mix])*sel,4)
